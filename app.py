...
def clean_val(val):
    """Убирает пустые двоеточия и красиво форматирует налоги/учредителей."""
    if not val: return ""
...
def process_contacts(df, col_name, prefix):
    """Разносит контакты по столбцам Телефон 1, Телефон 2 и т.д."""
...
def toggle_all_selection():
    """Коллбэк для чекбокса 'Выбрать все'."""
...
okved_input = st.sidebar.text_input("ОКВЭДы (через | )", placeholder="Пример: 62 или 62.01|63.11")
...
with st.sidebar.expander("💰 Финансы (млн руб.)", expanded=True):
    r_col1, r_col2 = st.columns(2)
    with r_col1:
        rev_min = st.number_input("Выручка от", value=0, key="rev_min", label_visibility="collapsed")
    with r_col2:
        rev_max = st.number_input("Выручка до", value=0, key="rev_max", label_visibility="collapsed")
    st.caption("Минимальная и максимальная выручка")

with st.sidebar.expander("👥 Параметры", expanded=True):
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if items:
                    ...
                    st.warning("Компаний не найдено.")
            elif r.status_code == 403:
                st.error(f"Ошибка 403: Добавьте IP {r.text} в белый список API.")
...
if 'results' in st.session_state and st.session_state['results'] is not None:
    ...
    st.metric("Найдено компаний", st.session_state['search_count'])
    ...
    if st.button(f"🚀 Обогатить данными ({len(selected)})", use_container_width=True):
        if selected.empty:
            ...
            if enriched:
                ...
                st.subheader("💎 Финальная база (Контакты + Финансы + Налоги)")
                st.dataframe(final_df, use_container_width=True)
                
                csv = final_df.to_csv(index=False).encode('utf-8-sig')
                st.download_button("📥 Скачать в Excel (CSV)", csv, "lead_export.csv", "text/csv")
